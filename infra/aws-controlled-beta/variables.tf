...
variable "image_digest" {
  ...
  validation { condition = can(regex("^sha256:[0-9a-f]{64}$", var.image_digest))
...
variable "github_repository" {
  ...
  validation { condition = can(regex("^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$", var.github_repository))
  error_message = "Use ORG/REPO." }
}
variable "github_oidc_provider_arn" { type = string }
variable "gateway_role_bindings_secret_arn" { type = string
  sensitive = true }
...
variable "desired_count" { type = number
  default = 2
  validation { condition = var.desired_count >= 2
