resource "aws_cognito_user_pool" "main" {
  ...
  account_recovery_setting { recovery_mechanism { name = "verified_email"
  priority = 1 } }
...
resource "aws_cognito_user_pool_domain" "main" { domain = var.cognito_domain_prefix
  user_pool_id = aws_cognito_user_pool.main.id }
...
resource "aws_iam_role" "github_plan" { name = "${local.name}-github-plan"
...
resource "aws_iam_role_policy" "github_plan" { name = "read-only-plan"
